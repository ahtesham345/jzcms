<x-layout.admin title="Edit Parent">
    <x-slot name="header">
        Edit Parent
    </x-slot>
    ...
    <div class="bg-white rounded-lg shadow-sm">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-800">Edit Parent - {{ $parent->parent_id }}</h3>
        </div>

        <form method="POST" action="{{ route('parents.update', $parent->id) }}" class="p-6" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center mb-3">
                        <div class="h-20 w-20 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden border border-gray-300">
                            @if($parent->hasPhoto())
                                <img src="{{ $parent->photoUrl() }}" class="h-20 w-20 rounded-full object-cover" alt="{{ $parent->full_name }}">
                            @else
                                <span class="text-xl font-medium text-gray-600">{{ $parent->initial() }}</span>
                            @endif
                        </div>
                        <p class="ml-4 text-sm text-gray-500">
                            {{ $parent->hasPhoto() ? 'Current photo. Uploading a new one replaces it.' : 'No photo uploaded yet.' }}
                        </p>
                    </div>
                ...
                <div>
                    ...
                    {{-- Not submitted: the field is disabled and the server
                         rejects a parent_id in the request regardless. --}}
                    <input type="text" id="parent_id_display" value="{{ $parent->parent_id }}" readonly disabled
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-600 cursor-not-allowed">
                    <p class="mt-1 text-sm text-gray-500">The parent ID is generated automatically and cannot be changed.</p>
                    @error('parent_id')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="full_name" id="full_name" value="{{ old('full_name', $parent->full_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="father_name" id="father_name" value="{{ old('father_name', $parent->father_name) }}"
                ...
                <div>
                    ...
                    <select name="gender" id="gender" required
                        ...
                        @foreach(\App\Models\ParentGuardian::GENDERS as $gender)
                            <option value="{{ $gender }}" {{ old('gender', $parent->gender) === $gender ? 'selected' : '' }}>{{ $gender }}</option>
                        @endforeach
                    </select>
                    @error('gender')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="cnic_number" id="cnic_number" value="{{ old('cnic_number', $parent->cnic_number) }}" placeholder="XXXXX-XXXXXXX-X"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="mobile_number" id="mobile_number" value="{{ old('mobile_number', $parent->mobile_number) }}" placeholder="03XX-XXXXXXX" required
                ...
                <div>
                    ...
                    <input type="text" name="alternate_mobile" id="alternate_mobile" value="{{ old('alternate_mobile', $parent->alternate_mobile) }}" placeholder="03XX-XXXXXXX"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" id="address" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('address') border-red-500 @enderror">{{ old('address', $parent->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="parent_status" id="parent_status" required
                        ...
                        @foreach(\App\Models\ParentGuardian::STATUSES as $statusOption)
                            <option value="{{ $statusOption }}" {{ old('parent_status', $parent->parent_status) === $statusOption ? 'selected' : '' }}>{{ $statusOption }}</option>
                        @endforeach
                    </select>
                    @error('parent_status')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4" placeholder="Any additional information..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror">{{ old('notes', $parent->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
