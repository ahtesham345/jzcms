<x-layout.admin title="Add Teacher">
    <x-slot name="header">
        Add Teacher
    </x-slot>
    ...
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <form method="POST" action="{{ route('teachers.store') }}" class="p-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    {{-- Preview of the next available ID, read from the same
                         Teacher::nextTeacherId() the backend uses. Disabled and
                         not submitted: the final ID is allocated on save. --}}
                    <input type="text" id="teacher_id_display" value="{{ $nextTeacherId }}" readonly disabled
                ...
                <div>
                    ...
                    <select name="teacher_status" id="teacher_status" required
                        ...
                        @foreach(\App\Models\Teacher::STATUSES as $statusOption)
                            <option value="{{ $statusOption }}" {{ old('teacher_status', 'Active') === $statusOption ? 'selected' : '' }}>{{ $statusOption }}</option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    @else
                            ...
                            @foreach($academicClasses->groupBy(fn ($class) => $class->department?->name ?? 'Unassigned') as $departmentName => $classes)
                                <optgroup label="{{ $departmentName }}">
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" {{ in_array($class->id, old('academic_class_ids', [])) ? 'selected' : '' }}>
                                            {{ $class->name }}
                                        </option>
                                    @endforeach
                                </optgroup>
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            Optional. Hold Ctrl (Cmd on Mac) to select more than one class.
                        </p>
                    @endif
