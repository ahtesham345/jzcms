    ...
    <div class="bg-white rounded-lg shadow-sm">
            ...
            <form method="GET" action="{{ route('teachers.index') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Search by teacher ID, name, or mobile number..."
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <!-- Status Filter -->
                    <div>
                        <label for="teacher_status" class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select
                            name="teacher_status"
                            id="teacher_status"
                        ...
                        >
                            ...
                            @foreach(\App\Models\Teacher::STATUSES as $statusOption)
                                <option value="{{ $statusOption }}" {{ request('teacher_status') === $statusOption ? 'selected' : '' }}>{{ $statusOption }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    @if(request()->hasAny(['search', 'teacher_status']))
                        <a
                            ...
                            <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <div class="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden">
                                                @if($teacher->hasPhoto())
                                                    <img src="{{ $teacher->photoUrl() }}" class="h-10 w-10 rounded-full object-cover" alt="{{ $teacher->full_name }}">
                                                @else
                                                    <span class="text-sm font-medium text-gray-700">{{ $teacher->initial() }}</span>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $teacher->full_name }}</div>
                                            <div class="text-sm text-gray-500">{{ $teacher->father_name ?? '—' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">
                                    ...
                                    @else
                                        ...
                                        <span title="{{ $teacher->academicClasses->pluck('name')->implode(', ') }}">
                                            {{ $teacher->academicClasses->take(2)->pluck('name')->implode(', ') }}@if($teacher->academicClasses->count() > 2)
                                                <span class="text-gray-500">+{{ $teacher->academicClasses->count() - 2 }}</span>
                                            @endif
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <a href="{{ route('teachers.show', $teacher->id) }}" class="text-blue-600 hover:text-blue-800" title="View Teacher">
                                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                        </svg>
                                    </a>
                                    <a href="{{ route('teachers.edit', $teacher->id) }}" class="text-yellow-600 hover:text-yellow-800" title="Edit Teacher">
                                        <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('teachers.destroy', $teacher->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this teacher?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-800" title="Delete Teacher">
                                            <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @else
            ...
            <div class="px-6 py-12 text-center">
                ...
                @if(request()->hasAny(['search', 'teacher_status']))
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No teachers found matching your criteria</h3>
